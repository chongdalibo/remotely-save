...
import type { RenameLedgerEntry } from "./renameLedger";
...
// Local-only, always-succeeds record of rename events this device has
// observed via Obsidian's own rename event, waiting to be merged into the
// shared remote rename ledger next time a sync actually runs (so a rename
// performed while offline isn't lost).
export const getPendingRenamesByVault = async (
  db: InternalDBs,
  vaultRandomID: string
): Promise<RenameLedgerEntry[]> => {
  const raw = await db.simpleKVForMiscTbl.getItem(
    `${vaultRandomID}-pendingRenames`
  );
  return (raw as RenameLedgerEntry[] | null | undefined) ?? [];
};

export const appendPendingRenameByVault = async (
  db: InternalDBs,
  vaultRandomID: string,
  entry: RenameLedgerEntry
) => {
  const existing = await getPendingRenamesByVault(db, vaultRandomID);
  existing.push(entry);
  await db.simpleKVForMiscTbl.setItem(
    `${vaultRandomID}-pendingRenames`,
    existing
...
// Removes only the given entries (by exact fromKey+toKey+when match), not a
// blind clear - so a rename queued concurrently with a sync round's flush
// isn't dropped.
export const removePendingRenamesByVault = async (
  db: InternalDBs,
  vaultRandomID: string,
  toRemove: RenameLedgerEntry[]
) => {
  const existing = await getPendingRenamesByVault(db, vaultRandomID);
  const removeSet = new Set(
    toRemove.map((e) => `${e.fromKey}\t${e.toKey}\t${e.when}`)
  );
  const remaining = existing.filter(
    (e) => !removeSet.has(`${e.fromKey}\t${e.toKey}\t${e.when}`)
  );
  await db.simpleKVForMiscTbl.setItem(
    `${vaultRandomID}-pendingRenames`,
    remaining
