...
export default class RemotelySavePlugin extends Plugin {
  ...
  pendingRenameAppendQueue: Promise<void> = Promise.resolve();
  ...
  async syncRun(triggerSource: SyncTriggerSourceType = "manual") {
    ...
    const statusBarFunc = async (
      ...
      if (step === 1) {
        ...
        this.updateLastSyncMsg(s, "not_syncing", s === "dry" ? null : ts, null); // hack: 'not_syncing'
  ...
  async onload() {
    ...
    if (!this.settings.agreeToUseSyncV3) {
      ...
      this.enableRenameLedgerTracking();
    }
  ...
  _onFileRenamed = async (file: TAbstractFile, oldPath: string) => {
    // Local-only, always succeeds regardless of network state - the actual
    // reporting to the shared remote rename ledger happens next time a sync
    // runs (see syncer() in pro/src/sync.ts), so a rename made while this
    // device is offline isn't lost.
    const entry = {
      fromKey: oldPath,
      toKey: file.path,
      when: Date.now(),
    };
    this.pendingRenameAppendQueue = this.pendingRenameAppendQueue
      .catch(() => undefined)
      .then(async () => {
        await appendPendingRenameByVault(this.db, this.vaultRandomID, entry);
      })
      .catch((e) => {
        console.error("remotely-save: failed to record pending rename", e);
      });
    await this.pendingRenameAppendQueue;
  };

  enableRenameLedgerTracking() {
    // Independent of "sync on save" (toggleSyncOnSaveIfSet): renames must be
    // recorded for the ledger regardless of whether that unrelated feature
    // is enabled.
    this.app.workspace.onLayoutReady(() => {
      this.registerEvent(this.app.vault.on("rename", this._onFileRenamed));
    });
  }
