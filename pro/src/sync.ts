...
import {
  ...
  getPendingRenamesByVault,
  insertSyncPlanRecordByVault,
  removePendingRenamesByVault,
...
// Tolerance used ONLY when deciding whether an entity that now exists on just
// one side (the other side vanished) is unchanged since prevSync, i.e. whether
// a deletion/rename on the other device should be trusted and propagated here.
// WebDAV (and some mobile filesystems) only round-trip mtime to whole-second
// precision (see fromWebdavItemToEntity in fsWebdav.ts, which derives mtimeCli
// from the HTTP Last-Modified header), so a strict === comparison spuriously
// treats an untouched file as "modified" and re-uploads/resurrects it instead
// of deleting it, once one side of the file has actually vanished. Deliberately
// NOT applied to the "both sides still exist" comparisons (branch 2 and the
// localEqualPrevSync/remoteEqualPrevSync checks below): those decide whether to
// silently do nothing, so a false "unchanged" there could swallow a real
// concurrent edit. Here, in contrast, one side is already gone, so the only
// choices are "propagate the deletion" or "resurrect it" - only the exact
// match ever caused the resurrection bug.
const MTIME_TOLERANCE_FOR_ONE_SIDE_DELETED_MS = 1500;

const isSameMTimeWithinDeleteTolerance = (
  a: number | undefined,
  b: number | undefined
) => {
  ...
  return Math.abs(a - b) <= MTIME_TOLERANCE_FOR_ONE_SIDE_DELETED_MS;
};
...
export const checkIsSkipItemOrNotByName = (
  ...
  const checkIsHidden =
    ...
    key === DEFAULT_FILE_NAME_FOR_RENAME_LEDGER;
  if (finalIsIgnored === undefined) {
...
export const getSyncPlanInplace = async (
  ...
  configDir: string,
  lastSuccessSyncMillis: number | undefined = undefined,
  treatAsFreshProfileSync = false,
  renameLedger: RenameLedger = emptyRenameLedger()
) => {
  profiler?.addIndent();
...
const splitFourStepsOnEntityMappings = (
  ...
  for (let i = 0; i < sortedKeys.length; ++i) {
    ...
    if (
      ...
      val.decision === "local_is_created_but_looks_stale_then_hold" ||
      val.decision === "local_is_modified_but_remote_missing_then_hold" ||
...
const dispatchOperationToActualV3 = async (
  ...
  if (r.decision === "only_history") {
    ...
    r.decision === "local_is_created_but_looks_stale_then_hold" ||
    r.decision === "local_is_modified_but_remote_missing_then_hold" ||
...
export async function syncer(
  ...
  try {
    ...
    // console.debug(prevSyncEntityList);
    // No prevSync record at all for this profile - either it was just
    // cleared via "clear sync records", or this is the first sync ever done
    // under this profile. Either way every local-only file legitimately
    // looks brand new to this profile, so the "stale orphan" suspicion below
    // must not apply this round.
    const treatAsFreshProfileSync = prevSyncEntityList.length === 0;
    const lastSuccessSyncMillis = await getLastSuccessSyncTimeByVault(
      db,
      vaultRandomID
    );

    // Rename ledger: fetch the shared remote copy, fold in any rename
    // events this device recorded locally (possibly while offline, via the
    // vault "rename" event) but hasn't reported yet, and prune old entries.
    // Read-only for "dry" runs - previewing a sync must not mutate remote
    // state or clear the pending queue.
    let renameLedger = await readRenameLedgerFromRemote(
      fsEncrypt,
      remoteEntityList
    );
    const pendingRenames = await getPendingRenamesByVault(db, vaultRandomID);
    if (pendingRenames.length > 0) {
      renameLedger = pruneRenameLedger(
        mergeRenameLedgerEntries(renameLedger, pendingRenames),
        Date.now()
      );
      if (triggerSource !== "dry") {
        await writeRenameLedgerToRemote(fsEncrypt, renameLedger);
        await removePendingRenamesByVault(db, vaultRandomID, pendingRenames);
      }
    }
    ...
    mixedEntityMappings = await getSyncPlanInplace(
      ...
      triggerSource,
      configDir,
      lastSuccessSyncMillis ?? undefined,
    ...
    const heldBackCount = Object.values(mixedEntityMappings).filter(
      (v) =>
        v.decision === "local_is_created_but_looks_stale_then_hold" ||
        v.decision === "local_is_modified_but_remote_missing_then_hold"
    ).length;
    if (heldBackCount > 0) {
      await errNotifyFunc?.(
        triggerSource,
        Error(
          `remotely-save: held back ${heldBackCount} local file(s) that look like stale offline copies ` +
            `instead of uploading them. Export the sync plan for details.`
        )
      );
    }
